...
source("~/Documents/GitHub/AntibodyForests/R/AntibodyForests_sync_nodes.R")
source("~/Documents/GitHub/AntibodyForests/R/AntibodyForests_plot.R")
source("~/Documents/GitHub/AntibodyForests/R/AntibodyForests_add_node_feature.R")
source("~/Documents/GitHub/AntibodyForests/R/AntibodyForests_PLM_plot.R")
source("~/Documents/GitHub/AntibodyForests/R/AntibodyForests_PLM_dataframe.R")
source("~/Documents/GitHub/AntibodyForests/R/AntibodyForests_distance_scatterplot.R")
source("~/Documents/GitHub/AntibodyForests/R/AntibodyForests_get_sequences.R")
```
<br>
...
### Synchronize node names of AntibodyForests objects with the `AntibodyForests_sync_nodes()` function
...
<br>
To achieve this, the AntibodyForests_sync_nodes() function can be used. This function takes one AntibodyForests object as a reference and renames the nodes of all clonotypes within all samples of another AntibodyForests object to match the reference. The function matches the nodes using barcodes.  
<br>
...
```{r AntibodyForests_sync_nodes, eval = FALSE}
AntibodyForests_OVA_IgPhyML <- AntibodyForests_sync_nodes(reference = AntibodyForests_OVA_default,
...
## 4. Quantifying evolution and comparing antibody lineage trees
...
## 5. Assessing evolutionary likelihood of somatic hypermutation with Protein Language Models (PLM)
...
<br>
The pseudolikelihoods and probability matrices can be supplied manually, or easily generated using the [PLM-pipeline](https://github.com/dvginneken/PLM-pipeline). Here, a VDJ-dataframe can be used as input CSV file, and the pipeline can generate the pseudolikelihoods and probability matrices of those sequences using different PLMs. The resulting pseudolikelihoods can be incoporates as node feature in the AntibodyForests-object using the **`AntibodyForests_add_node_feature()`** function. The pseudolikelihoods can be visualized using the **`AntibodyForests_plot()`** function, and the correlation with distance to the germline can be visualized using the **`AntibodyForests_distance_scatterplot`** function.  
<br>
 Sequences corresponding to nodes in the AntibodyForests-object can be extracted using the **`AntibodyForests_get_sequences()`** function. This dataframe can be used as input CSV file for the [PLM-pipeline](https://github.com/dvginneken/PLM-pipeline) to be able to match probability matrices to the right trees and nodes. PLM-generated probability matrices can be supplied to the function **`AntibodyForests_PLM_dataframe()`** to generate a dataframe with the ranks and probabilities of the mutations along the edges of the lineage trees. These ranks and probabilties of both the mutated amino acids and the original amino acid can be visualized using the **`AntibodyForests_PLM_plot()`** function.  
<br>
...
#### **AntibodyForests_add_node_feature()**
...
#### **AntibodyForests_distance_scatterplot()**
...
* **Description:** Name of the sequence column in the AntibodyForests object as obtained from the **`AntibodyForests()`** function.  
...
#### **AntibodyForests_PLM_plot()**

`PLM_dataframe`

* **Description:** A dataframe as obtained from the **`AntibodyForests_PLM_dataframe()`** function.
...
AntibodyForests_OVA_default <- AntibodyForests_add_node_feature(AntibodyForests_object = AntibodyForests_OVA_default, 
                                                                feature.df = VDJ_OVA,
...
AntibodyForests_distance_scatterplot(AntibodyForests_object = AntibodyForests_OVA_default, 
...
df <- AntibodyForests_get_sequences(AntibodyForests_object = AntibodyForests_OVA_default, 
...
PLM_dataframe <- AntibodyForests_PLM_dataframe(AntibodyForests_object = AntibodyForests_OVA_default, 
                                               sequence.name = "VDJ_sequence_aa_trimmed", 
...
AntibodyForests_PLM_plot(PLM_dataframe = PLM_dataframe, 
...
## 6. Investigate the evolution of antibody structure along the lineage trees
...
The AntibodyForests package originated from the [Platypus](https://academic.oup.com/bioinformatics/article/39/9/btad553/7264179) ecosystem of packages, where the fundamental function `AntibodyForests()` was initially developed. Subsequently, significant refinements were made, and they were migrated to this standalone package. 
