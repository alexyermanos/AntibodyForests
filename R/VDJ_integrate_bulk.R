...
VDJ_integrate_bulk <- function(sc.VDJ,
  ...
  if(operating.system == 'Windows'){os_prefix <- 'cmd.exe '}else{os_prefix <- ''}
  ...
  Transform_to_VDJ <- function(annotations_combined, sc.VDJ, bulk.tsv, bulk.tsv.sample.column, bulk.tsv.barcode.column,
                               bulk.tsv.sequence.column, bulk.tsv.isotype.column){
    ...
    VDJ_combined_unclonotyped_unfiltered <- do.call(rbind, lapply(unique(sc.VDJ$sample_id), function(sample){
      ...
      VDJ_combined_unclonotyped_unfiltered_sample <- lapply(1:nrow(sample_combined), function(row){
        ...
        VDJ_combined_unclonotyped_row <- data.frame(sample_id = sample,
                                                    ...
                                                    VDJ_sequence_nt_trimmed = paste0(IgBLAST_annotations$fwr1,
                                                                                     IgBLAST_annotations$cdr1,
                                                                                     IgBLAST_annotations$fwr2,
                                                                                     IgBLAST_annotations$cdr2,
                                                                                     IgBLAST_annotations$fwr3,
                                                                                     IgBLAST_annotations$cdr3,
                                                                                     IgBLAST_annotations$fwr4),
                                                    VDJ_sequence_aa_trimmed = paste0(IgBLAST_annotations$fwr1_aa,
                                                                                     IgBLAST_annotations$cdr1_aa,
                                                                                     IgBLAST_annotations$fwr2_aa,
                                                                                     IgBLAST_annotations$cdr2_aa,
                                                                                     IgBLAST_annotations$fwr3_aa,
                                                                                     IgBLAST_annotations$cdr3_aa,
  ...
  Bulk_Clonotyping <- function(VDJ_combined, tie.resolvement, seq.identity){
    ...
    for (clonotype in unique(VDJ_sample$clonotype_id)){
      nt_germline <- sort(table(unique(VDJ_sample[VDJ_sample$clonotype_id == clonotype & VDJ_sample$dataset == "single-cell", "VDJ_germline_nt_trimmed"])), decreasing = T)[1]
      aa_germline <- sort(table(unique(VDJ_sample[VDJ_sample$clonotype_id == clonotype & VDJ_sample$dataset == "single-cell", "VDJ_germline_aa_trimmed"])), decreasing = T)[1]
      VDJ_sample[VDJ_sample$clonotype_id == clonotype & VDJ_sample$dataset == "bulk", "VDJ_germline_nt_trimmed"] <- names(nt_germline)
      VDJ_sample[VDJ_sample$clonotype_id == clonotype & VDJ_sample$dataset == "bulk", "VDJ_germline_aa_trimmed"] <- names(aa_germline)
    }
  ...
  if(is.null(bulkRNA_seqs_annotations) | is.null(scRNA_seqs_annotations)){
   ...
   # 2. Run IgBLAST to annotate the single cell and bulk sequences
    ...
    scRNA_seqs_annotations <- read.table(paste0(temp_dir, "/unique_scRNA_seqs_igblast.tsv"), header = TRUE, sep = "\t")
    bulkRNA_seqs_annotations <- read.table(paste0(temp_dir, "/unique_bulkRNA_seqs_igblast.tsv"), header = TRUE, sep = "\t")
    unlink(temp_dir, recursive = T)
  }
  else{
    # Read the IgBLAST annotations for single-cell and bulk sequences
    scRNA_seqs_annotations <- read.table(scRNA_seqs_annotations, header = TRUE, sep = "\t")
    bulkRNA_seqs_annotations <- read.table(bulkRNA_seqs_annotations, header = TRUE, sep = "\t")
  }
  ...
  VDJ_combined <- dplyr::left_join(VDJ_combined[,!(colnames(VDJ_combined) %in% c("clonotype_id", "clonotype_frequency","VDJ_germline_nt_trimmed", 
                                                                                 "VDJ_germline_aa_trimmed", "VJ_germline_aa_trimmed", "VJ_germline_nt_trimmed"))], 
                                   sc.VDJ[,c("sample_id", "barcode", "VDJ_contig_id", "clonotype_id", "clonotype_frequency", "VDJ_germline_nt_trimmed", 
                                             "VDJ_germline_aa_trimmed", "VJ_germline_aa_trimmed", "VJ_germline_nt_trimmed")], 
