...
output:
  html_vignette:
    ...
    toc: true
vignette: >
  %\VignetteIndexEntry{AntibodyForests vignette: a comprehensive guide for importing 10x single-cell V(D)J sequencing data, building lineage trees, and comparing trees within and across repertoires}
...
![](imgs/main/workflow.png){width=100%}
...
load("objects/quick-start_out.RData")
load("objects/quick-start_plots.RData")
```
...
```
![](imgs/trees/quick-start_tree.png){width=80%}
...
```{r, quick-start4, fig.width=4, fig.height=5}
...
```{r, compare_distance_isotype, fig.width=4, fig.height=5}
# Cluster the trees in the repertoire based on the euclidean distance between various metrics of each tree
...
```{r, cluster_metrics_example1, fig.width=4, fig.height=5}
# Analyze the difference between the clusters based on the calculated sackin.index
plots <- Af_cluster_metrics(input = AntibodyForests_OVA_default,
                   clusters = out$clustering,
...
```{r, cluster_node_features_example1, fig.width=4, fig.height=5}
# Analyze the difference between the clusters based on the calculated sackin.index
...
```{r, compare_within_repertoire_example2, fig.width=8, fig.height=10}
...
```{r, cluster_metrics_example2, fig.width=4, fig.height=5}
# Analyze the difference between the clusters based on the calculated sackin.index
plots <- Af_cluster_metrics(input = AntibodyForests_OVA_default,
                   clusters = out$clustering,
...
```{r, cluster_node_features_example2, fig.width=5, fig.height=4}
# Analyze the difference between the clusters based on the calculated sackin.index
plots <- Af_cluster_node_features(input = AntibodyForests_OVA_default,
                   clusters = out$clustering,
                   features = "isotype",
...
load("objects/AntibodyForests_example_blood.RData")
load("objects/AntibodyForests_example_LN.RData")
```
...
```{r, compare_across_repertoire_example1, fig.width=4, fig.height=5}
...
```{r, compare_across_repertoire_example2, fig.width=6, fig.height=5}
#Calculate the difference between the two groups of repertoires
...
```{r, plot_likelihood_example1, fig.width=5, fig.height=5}
...
```{r, plot_likelihood_example2, fig.width=5, fig.height=5}
# Plot the substitution rank
Af_plot_PLM(PLM_dataframe = PLM_dataframe, 
...
```{r, plot_likelihood_example3, fig.width=5, fig.height=5}
# Plot the substitution rank
Af_plot_PLM(PLM_dataframe = PLM_dataframe, 
...
```{r, plot_likelihood_example4, fig.width=5, fig.height=5}
# Plot the substitution rank
Af_plot_PLM(PLM_dataframe = PLM_dataframe, 
...
rmsd_df <- Af_edge_RMSD(af,
             ...
             chain = "HC+LC")
```
![](imgs/structure_Ab_distance.png){width=60%}
...
rmsd_df <- Af_edge_RMSD(af,
             ...
             chain = "whole.complex")
```
![](imgs/structure_AbAg_distance.png){width=60%}
