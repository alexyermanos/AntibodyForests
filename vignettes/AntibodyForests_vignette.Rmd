...
knitr::opts_knit$set(root.dir = "C:/Users/Valen/OneDrive/Documenten/Studie/Master Infection and Immunity/Major Research Project - Alex Yermanos/Computations/vignettes/")
# Set global options
...
source("../scripts/VDJ_build.R")
source("../scripts/VDJ_import_IgBLAST_annotations.R")
source("../scripts/VDJ_to_AIRR.R")
source("../scripts/AntibodyForests.R")
source("../scripts/AntibodyForests_sync.R")
source("../scripts/AntibodyForests_plot.R")
```
<br>
...
#### **Example 1**  
In the example provided below, all transcripts are imported into the VDJ dataframe without undergoing any filtering steps. The resulting dataframe comprises a total of 22,425 cells. It is worth noting that individual cells may contain multiple VDJ and VJ transcripts within a single row, and a few cells may lack either the VDJ or VJ transcript. For instance, multiple cells of clonotype5 of S1 have multiple VJ transcripts, including those with barcodes ACATCAGAGCAATCTC, AGCAGCCTCTGTCTAT, and AGCGGTCGTCCAACTA. Additionally, multiple cells of clonotype10 of S1 lack a VDJ transcript, including those with barcodes AACTGGTGTGCCTGCA, AAGGAGCAGTACGTTC, and ACAGCCGAGACTAGAT.
```{r vdj_build-example-1-code, results = "asis", message = TRUE}
...
```{r vdj_build-example-1-message, echo = FALSE}
# Print number of rows in the 'VDJ_OVA' dataframe
message(paste("Total number of rows (i.e. cells) in the 'VDJ_OVA' dataframe:", nrow(VDJ_OVA)))
```
<br>

An example VDJ dataframe is provided below, containing the data of clonotype 1-10 of all samples:
```{r vdj_build-example-1-df, echo = FALSE}
# Print first 10 clonotypes of all samples
DT::datatable(data = VDJ_OVA[VDJ_OVA$clonotype_id %in% paste0("clonotype", 1:10), ],
              rownames = FALSE,
...
#### **Example 2**  
In the upcoming example, cells with multiple VDJ or VJ transcripts, referred to as 'divergent cells', are filtered out. The left column of the printed table displays the count of divergent cells filtered out for each sample.  
```{r vdj_build-example-2-code, results = "asis", message = TRUE}
...
```{r vdj_build-example-2-message, echo = FALSE}
# Print number of rows in the 'VDJ_OVA' dataframe
message(paste("Total number of rows (i.e. cells) in the 'VDJ_OVA' dataframe:", nrow(VDJ_OVA)))
```
<br>
...
```{r vdj_build-example-3-message, echo = FALSE}
# Print number of rows in the 'VDJ_OVA' dataframe
message(paste("Total number of rows (i.e. cells) in the 'VDJ_OVA' dataframe:", nrow(VDJ_OVA)))
```
<br>
...
# Install OpenMP-enabled C Compiler
...
mkdir tools
wget https://bitbucket.org/kleinstein/immcantation/src/89303467be8cf07dcacf33fb5647e5fc28c3a670/scripts/fetch_igblastdb.sh -o tools/fetch_igblastdb.sh
wget https://bitbucket.org/kleinstein/immcantation/src/4b225480cf9338a4d991c6c14894570cfb5591c2/scripts/fetch_imgtdb.sh -o tools/fetch_imgtdb.sh
wget https://bitbucket.org/kleinstein/immcantation/src/eed7becf0e15fa629a37211e2d0091a0ef80f77d/scripts/clean_imgtdb.py -o tools/clean_imgtdb.py
wget https://bitbucket.org/kleinstein/immcantation/src/20906ed10968abc8ccc762a7a9ff83a03b32ef62/scripts/imgt2igblast.sh -o tools/imgt2igblast.sh
...
AssignGenes.py igblast -s ../data/OVA_scRNA-seq_data/VDJ/S1/filtered_contig.fasta -b anaconda3/envs/igphyml/share/igblast --organism mouse --loci ig --format blast
AssignGenes.py igblast -s ../data/OVA_scRNA-seq_data/VDJ/S2/filtered_contig.fasta -b anaconda3/envs/igphyml/share/igblast --organism mouse --loci ig --format blast
AssignGenes.py igblast -s ../data/OVA_scRNA-seq_data/VDJ/S3/filtered_contig.fasta -b anaconda3/envs/igphyml/share/igblast --organism mouse --loci ig --format blast
AssignGenes.py igblast -s ../data/OVA_scRNA-seq_data/VDJ/S4/filtered_contig.fasta -b anaconda3/envs/igphyml/share/igblast --organism mouse --loci ig --format blast
AssignGenes.py igblast -s ../data/OVA_scRNA-seq_data/VDJ/S5/filtered_contig.fasta -b anaconda3/envs/igphyml/share/igblast --organism mouse --loci ig --format blast

# Create database files to store sequence alignment information with the IMGT reference sequences
MakeDb.py igblast -i ../data/OVA_scRNA-seq_data/VDJ/S1/filtered_contig_igblast.fmt7 -s ../data/OVA_scRNA-seq_data/VDJ/S1/filtered_contig.fasta -r anaconda3/envs/igphyml/share/germlines/imgt/mouse/vdj/imgt_mouse_*.fasta --10x ../data/OVA_scRNA-seq_data/VDJ/S1/filtered_contig_annotations.csv --extended
MakeDb.py igblast -i ../data/OVA_scRNA-seq_data/VDJ/S2/filtered_contig_igblast.fmt7 -s ../data/OVA_scRNA-seq_data/VDJ/S2/filtered_contig.fasta -r anaconda3/envs/igphyml/share/germlines/imgt/mouse/vdj/imgt_mouse_*.fasta --10x ../data/OVA_scRNA-seq_data/VDJ/S2/filtered_contig_annotations.csv --extended
MakeDb.py igblast -i ../data/OVA_scRNA-seq_data/VDJ/S3/filtered_contig_igblast.fmt7 -s ../data/OVA_scRNA-seq_data/VDJ/S3/filtered_contig.fasta -r anaconda3/envs/igphyml/share/germlines/imgt/mouse/vdj/imgt_mouse_*.fasta --10x ../data/OVA_scRNA-seq_data/VDJ/S3/filtered_contig_annotations.csv --extended
MakeDb.py igblast -i ../data/OVA_scRNA-seq_data/VDJ/S4/filtered_contig_igblast.fmt7 -s ../data/OVA_scRNA-seq_data/VDJ/S4/filtered_contig.fasta -r anaconda3/envs/igphyml/share/germlines/imgt/mouse/vdj/imgt_mouse_*.fasta --10x ../data/OVA_scRNA-seq_data/VDJ/S4/filtered_contig_annotations.csv --extended
MakeDb.py igblast -i ../data/OVA_scRNA-seq_data/VDJ/S5/filtered_contig_igblast.fmt7 -s ../data/OVA_scRNA-seq_data/VDJ/S5/filtered_contig.fasta -r anaconda3/envs/igphyml/share/germlines/imgt/mouse/vdj/imgt_mouse_*.fasta --10x ../data/OVA_scRNA-seq_data/VDJ/S5/filtered_contig_annotations.csv --extended
```
<br>
